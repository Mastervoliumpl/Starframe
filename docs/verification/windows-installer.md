...
Issue [#74](https://github.com/Mastervoliumpl/Starframe/issues/74) extends the existing NSIS installer with the accepted portrait artwork, Segoe UI body text, Bahnschrift headings with a system fallback, navy surfaces and orange headings/progress. Navigation buttons and the title bar use Windows dark styling. The license, destination field and details list use readable light text on navy. License URLs remain selectable plain text so RichEdit does not force blue links with insufficient contrast. No new installer framework, plugin or redistributed Windows font was added.
...
The updated test uninstaller was opened for the owner's visual/keyboard check. That result and final approval of the revised controls remain pending; #74 stays open. The fixture uses only ignored test evidence and a separate product/data identity. No real game or personal Starframe data was changed. Logs and any screenshots containing local paths remain ignored.
