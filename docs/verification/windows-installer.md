...
#27 retains the confirmed maintenance-uninstall flow correction and installed desktop first-start verification after migration. The cross-build fixture above tests migration through the shared storage opening path using finite maintenance commands. Moving final acceptance does not mark these remaining functional checks complete.
