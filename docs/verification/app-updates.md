...
- The release-profile installer migrated the retained schema-12 baseline directly to schema 14 and reopened it successfully. Existing records, artifact bytes and original sources matched; the two new tables were empty and a complete schema-12 backup remained. This uses the isolated installer-test identity and finite maintenance entry point.
...
- A locked-database regression verifies that a failed save cannot change the confirmed update channel or dismiss a notice. The backend adopts new preferences only after SQLite commits them.
