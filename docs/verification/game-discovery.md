...
The initial hosted run passed Rust checks and both executable builds, then failed to connect to WebView2 before the test startup deadline. CI now checks the Evergreen Runtime before compiling and installs Microsoft's signed bootstrapper only when it is absent. Native tests use isolated WebView2 profiles, retain startup output and allow 30 seconds for first startup; the 100 ms interaction target is unchanged. Runtime detection follows [Microsoft's distribution guidance](https://learn.microsoft.com/en-us/microsoft-edge/webview2/concepts/distribution).
