...
On 15 September 2026, the hosted audit found [RUSTSEC-2026-0285](https://rustsec.org/advisories/RUSTSEC-2026-0285.html), published on 14 September. Rustls 0.23.44 accepted TLS 1.3 handshake messages across encryption-level changes in a record. The handshake transcript remains authenticated; this finding does not establish that an attacker can alter or complete a handshake. Rustls is present through `tough` in the Windows dependency graph. The lockfile now selects patched 0.23.45, and the notice inventory records its exact archive hash and unchanged license texts. Cargo also resolved `tempfile`'s allowed `getrandom` range to the already locked 0.3.4. Full Rust formatting, Clippy and tests, the release/NSIS build, all 27 Python checks and installer notice/privacy preflight passed locally. Hosted audits must pass on the updated commit before distribution.
