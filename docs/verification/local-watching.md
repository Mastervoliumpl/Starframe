...
On 15 September 2026, hosted run 35020466822 reached the local import while automatic runtime preparation was still active. Its default five-second visibility assertion expired before the imported row appeared. The initial import now uses the same forty-second operation bound as the workflow's rebuild/deployment assertions. The complete native watched-source workflow passed locally against a freshly built debug executable. Required checks 35024265067 and dependency audits 35024265072 then passed on commit `bf70b4e`. This changes test synchronization, not product behavior.
