...
## Current milestone handoff: 0.7.0, 24 September 2026
...
The registry download boundary binds a verified release to an account grant, streams and hashes a bounded archive, renews interrupted grants, handles server progress rejection, and retains an account-bound receipt claim for retry after restart. Schema 17 holds those claims. A signed post-transfer recheck stops a changed or blocked release before promotion. These methods and isolated fixtures are recorded in [download verification](verification/registry-downloads.md). They are not yet connected to the desktop Downloads queue or an install plan; no registry archive is installed by this work.
...
The remaining order is #84/#85 integration, then #86 after website #101, followed by #87, #88, #89 and #90. #66, #82 and #83 also need final issue review and checked-head evidence. Reuse local and hosted checks for unchanged paths, but require all checks on the exact final PR head and then merged main before closing the milestone. Keep application update signing separate from registry trust. The existing installer remains unpublished.
