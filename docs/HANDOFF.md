...
The historical 0.0.1 contract below remains a record of the initial decisions. For current work, use [DESIGN.md](../DESIGN.md), [ARCHITECTURE.md](../ARCHITECTURE.md), [ROADMAP.md](../ROADMAP.md) and the active 0.7.0 issues. Development is on `codex/roadmap-native-integration-backlog` in [draft PR #78](https://github.com/Mastervoliumpl/Starframe/pull/78), at product version `0.7.0-dev.1`. The PR is open; no 0.7.0 merge, installer publication, website deployment or real-game mutation has occurred.

The shared Rust backend and internal headless command now exercise local imports, collections, package preparation and fake-game setup/launch without a webview; see [headless verification](verification/backend-headless.md). The desktop has a native Steam manager sign-in flow with Windows protected credential storage and isolated fixture checks; see [authentication verification](verification/registry-auth.md). Registry v1 transport and native identities are pinned to website contract `fcd81be6667e2595166698178d5f55074c5ee92a`; see [contract verification](verification/registry-contract.md). Signed keysets, releases and security decisions persist with rollback protection. Known hash blocks apply offline to existing package, enable and launch gates; see [trust verification](verification/registry-trust.md).

The registry download boundary binds a verified release to an account grant, streams and hashes a bounded archive, renews interrupted grants, handles server progress rejection, and retains an account-bound receipt claim for retry after restart. Schema 17 holds those claims. A signed post-transfer recheck stops a changed or blocked release before promotion. The desktop command and shared three-operation Downloads queue now handle registry archive transfer, cancellation, cache reuse, status and receipt retry. Cache checks and final archive hashing run off the storage worker. These methods and isolated fixtures are recorded in [download verification](verification/registry-downloads.md). Archive saving remains separate from installation; the declared install action and its fixture evidence are recorded below.

Website issue [#101](https://github.com/Mastervoliumpl/Starframe-Web/issues/101) is complete through [PR #113](https://github.com/Mastervoliumpl/Starframe-Web/pull/113), merged as `1d616d43d90227ee9967091be039e6c3e695091e`. Installation schema 1 declares Lua/BepInEx entry points and Map/AI source folders and logical destinations inside full signed release schema 2. The manager now parses and verifies four website-reference signed declarations from that exact revision. Unknown versions, missing plans and tampering fail. The shared queue now prepares declared packages and atomically commits exact registry library records in schema 18. The loopback map install, restart/cache reuse, failed-transaction rollback and distinct approvals over shared bytes passed. Exact Map/AI sources now join the shared engine-rooted deployment owner; fake-game placement, conflict retention, failed-removal rollback and interrupted-removal recovery passed. Explicit native selection now prepares all four formats through the shared setup backend with schema 4 Code activation. Its isolated fixture passed offline reopening, repeat setup, cancellation, readiness and owned removal. Explicit mixed local/registry selections now share ordering and setup with bounded disabled inventory. Local import and offline reopening fixtures passed without converting catalog IDs. Persisted native collection selection, sharing and UI wiring remain under #87–#88. See [installation verification](verification/registry-installation.md). AI placement is `<game>/engine/LJ/lua/AI/mods/<folder>` by owner decision; AI loading/gameplay and map gameplay remain unverified. The website checkout stays read-only.

The owner pointed to the existing signing backup. Its public catalog root is RSA/TUF, not the separate Ed25519 registry root. Production registry access remains unavailable. The current goal explicitly allows implementation and local acceptance with isolated synthetic roots; production provisioning is a separate prerequisite. No private key or Steam credential was read or used.

#83 and #84 are closed. Earlier head `0ea309d` passed [required checks 36075417218](https://github.com/Mastervoliumpl/Starframe/actions/runs/36075417218) and [audits 36075417056](https://github.com/Mastervoliumpl/Starframe/actions/runs/36075417056). The declaration and preparation increments are `4a38e4b` and `8acb149`; the latter passed [required checks 36247353241](https://github.com/Mastervoliumpl/Starframe/actions/runs/36247353241) and [audits 36247353023](https://github.com/Mastervoliumpl/Starframe/actions/runs/36247353023). Queue head `f051fcb` passed [required checks 36248249648](https://github.com/Mastervoliumpl/Starframe/actions/runs/36248249648) and [audits 36248249498](https://github.com/Mastervoliumpl/Starframe/actions/runs/36248249498) passed. The content-placement increment passed Rustfmt/Clippy, 174 library tests and the 19-test deployment slice. The queue increment passed local Rustfmt/Clippy, 171 library tests plus the executable/integration suites, and frontend checks with 12 Vitest tests. One existing isolated credential-removal assertion failed in the first full run; that exact fixture and the full suite passed unchanged on rerun. The failure was not reproduced or assigned a cause. Dependency head `290bb5c` passed required checks 36249804671 and audits 36249804531. Activation head `5d9c9a5` passed required checks 36251384005 and audits 36251383848. Activation increment `5d9c9a5` passed Rustfmt/Clippy, 179 library tests with seven documented ignored tests, 11 executable tests, the configuration/headless/runtime-contract suites, the explicit isolated setup test, and the pinned .NET build/111 runtime tests. #85 retains archive/download work and awaits the native browser entry point and final fixture acceptance; #86 installation, #87 dependencies/collections, #88 Mods browser, #89 catalog removal and #90 integration remain open. #66 and #82 need final issue review. Reuse valid evidence for unchanged paths and require checks on the exact final head and merged main before milestone closeout. Keep application update signing separate. The installer remains unpublished.
...
Package scope includes Starframe-managed DLLs, conventional BepInEx DLL plugins through a verified compatibility path, Lua-only content, map-only content and mixed code/content packages. Support is advertised per verified format and game integration; a content-only package must not need an artificial DLL. AI-specific packages remain deferred as described below.

Map content and map-support plugins need separate deployment and activation handling. [Reviewed map sources](planning/map-support.md) describe `.sanmap` content folders and an optional BepInEx plugin for Lua access to map-local files. Issue #13 must verify the relevant paths and behavior on the current game build, and review the DLL-required activation contract before adding content-only support. Source inspection alone does not establish successful in-game loading.

The owner deferred AI-package support on 6 September 2026. Their developer-message notes describe modifications to the shipped Lua AI and report no current in-game AI selector. These notes are background evidence, not installation instructions for Starframe. Do not implement AI selection or automated replacement of shipped AI code in the current milestones. Revisit the scope when the game provides suitable extension/selection facilities; no delivery version is assigned.
