...
## Current milestone handoff: 0.7.0, 25 September 2026
...
Website issue [#101](https://github.com/Mastervoliumpl/Starframe-Web/issues/101) remains open. It must publish author-declared code entry points and AI/map placement in the approved signed release before manager #86 can consume a plan. Website main `3f983dd` adds a primary `modType` for discovery but no signed install plan; [the contract record](verification/registry-contract.md) identifies this later revision separately from the pinned release/download fixtures. The website checkout is read-only here. The manager contract request is recorded on [#101](https://github.com/Mastervoliumpl/Starframe-Web/issues/101#issuecomment-5822499405). Do not infer an install plan from type, tags or archive names. No independent production registry root has been supplied, so production downloads remain unavailable; synthetic roots and credentials are confined to tests.

The next external prerequisite is website #101's committed signed install plan for manager #86. #87–#90 follow #86; #88 will expose registry releases in the native Mods browser. #85's transfer is locally verified but needs that release entry point, independent production trust and final desktop acceptance before it can close. #83 sign-in acceptance passed on draft PR head `07cbe03`; [checks run 36071120326](https://github.com/Mastervoliumpl/Starframe/actions/runs/36071120326) and [audits run 36071120073](https://github.com/Mastervoliumpl/Starframe/actions/runs/36071120073) passed. #84 trust uses isolated signed roots and blocks production access until a separate root is provisioned; [trust verification](verification/registry-trust.md) records its acceptance. #66 and #82 need final issue review. The account-switch increment is `d8418b1`; the queue increment is `91afee6`. Six registry queue fixtures, the full Windows Rust suite, Rustfmt and Clippy passed locally on `91afee6`. Reuse local and hosted checks for unchanged paths, but require all checks on the exact final PR head and then merged main before closing the milestone. Keep application update signing separate from registry trust. The existing installer remains unpublished.
