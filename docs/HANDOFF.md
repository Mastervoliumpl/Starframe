...
Website issue [#101](https://github.com/Mastervoliumpl/Starframe-Web/issues/101) remains open. It must publish author-declared code entry points and AI/map placement in the approved signed release before manager #86 can consume a plan. The website checkout is read-only here. The manager contract request is recorded on [#101](https://github.com/Mastervoliumpl/Starframe-Web/issues/101#issuecomment-5822499405). Do not infer an install plan from categories or archive names. No independent production registry root has been supplied, so production downloads remain unavailable; synthetic roots and credentials are confined to tests.

The next external prerequisite is website #101's committed signed install plan for manager #86. #87–#90 follow #86; #88 will expose registry releases in the native Mods browser. #85's transfer is locally verified but needs that release entry point, independent production trust and final desktop acceptance before it can close. #66, #82, #83 and #84 need final issue review and checked-head evidence. The account-switch increment is `d8418b1`; the queue increment is `91afee6`. Six registry queue fixtures, the full Windows Rust suite, Rustfmt and Clippy passed locally on `91afee6`. Hosted [checks run 36070623491](https://github.com/Mastervoliumpl/Starframe/actions/runs/36070623491) and [audits run 36070623136](https://github.com/Mastervoliumpl/Starframe/actions/runs/36070623136) started for that code head; the audits passed and Windows checks were still running at this handoff edit. Reuse local and hosted checks for unchanged paths, but require all checks on the exact final PR head and then merged main before closing the milestone. Keep application update signing separate from registry trust. The existing installer remains unpublished.
