...
For the pre-merge 0.6.0 rehearsal, the existing Checks workflow accepts `draft_release_commit` on `codex/0.6.0-windows-alpha`. The caller waits for its own Required checks job before calling the same release workflow; it also retains the prior checked-commit, identity and approval gates. Temporarily permitting that exact branch in the release environment requires owner approval. Remove its environment branch policy after the rehearsal. Normal release dispatch is restricted to main. The rehearsal creates a development draft, never a public release.
