...
The test creates disposable RSA keys, verifies the initial publication and renewal, and rejects a wrong signing key, invalid advisory data and modified previously published targets. It also checks recovery from a correctly signed but expired repository. A real Git round trip verifies that signed bytes survive Windows checkout: existing publications clone with `core.autocrlf=false`, and new publications include `* -text` attributes. The fixture keys are removed with their temporary directory.
