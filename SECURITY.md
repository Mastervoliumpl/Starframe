...
Use the maintained TUF client selected in the [authentication evaluation](docs/verification/catalog-authentication.md) with static GitHub hosting. Separate catalog approval authority from application release authority. On 9 September 2026 the owner chose daily renewal through GitHub Actions with thirty-day validity. Expired catalog metadata must pause new catalog downloads until a valid signed refresh succeeds. Preserve installed offline use, show the stale/unknown status, and retain cached confirmed security blocks. A mod's age or lack of new releases is not a security finding. A signed but stale catalog cannot reveal warnings the client has never received. The verification layer is implemented; the desktop refresh and policy integration remain in progress under #46.

Security advisories identify exact release IDs and artifact hashes, evidence, confirmation state, recommended action and correction history. The implemented validator also matches known payload hashes in local copies, requires appended corrections and rejects silently removed findings. Schema 13 can commit verified catalog and advisory data together and retain findings after expiry. Download, enable and activation checks enforce retained confirmed findings, including local copies; desktop delivery and warning display remain unfinished. Suspected findings do not trigger those blocks. Preserve files/settings, display a prominent warning, and do not mutate a running game. Cached confirmed findings remain effective offline; ordinary withdrawal must not stand in for them. Starframe cannot stop code already executing or prevent launches outside its own launch path.
