...
Evaluate maintained signing/TUF tooling and static GitHub hosting. Separate catalog approval authority from application release authority. Choose a practical renewal interval before enforcing expiry; no 90-day policy has been adopted. On 9 September 2026 the owner decided that expired catalog metadata must pause new catalog downloads until a valid signed refresh succeeds. Preserve installed offline use, show the stale/unknown status, and retain cached confirmed security blocks. A mod's age or lack of new releases is not a security finding. A signed but stale catalog cannot reveal warnings the client has never received. This policy is required for #46; the current unsigned catalog client does not implement it yet.
