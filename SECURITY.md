...
Security advisories identify exact release IDs and artifact hashes, evidence, confirmation state, recommended action and correction history. The implemented validator also matches known payload hashes in local copies, requires appended corrections and rejects silently removed findings. Schema 13 can commit verified catalog and advisory data together and retain findings after expiry. Desktop delivery and enforcement remain unfinished. The required behavior is to mark suspected findings as unconfirmed and block confirmed malicious artifacts from new downloads and activation through Starframe, including matching installed copies. Preserve files/settings, display a prominent warning, and do not mutate a running game. Cached confirmed findings must remain effective offline; ordinary withdrawal must not stand in for them.
