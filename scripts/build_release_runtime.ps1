...
try {
    dotnet build runtime/Starframe.Bootstrap/Starframe.Bootstrap.csproj --configuration Release -p:RestoreLockedMode=true --output "$taskOutput/build" "-p:GameManagedPath=$GameManagedPath" "-p:BootstrapPath=$BootstrapPath" "-p:PathMap=$taskRoot=/_/" -p:ContinuousIntegrationBuild=true
