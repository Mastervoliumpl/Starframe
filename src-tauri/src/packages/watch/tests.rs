...
fn migration_watches_unambiguous_sources_and_reimport_selects_an_ambiguous_source() {
    for multiple in [false, true] {
        ...
        db.execute_batch(
            "DROP TABLE registry_trust_streams; DROP TABLE registry_decisions; DROP TABLE registry_library; DROP TABLE app_updates; DROP TABLE catalog_security; DROP TABLE local_watches; PRAGMA user_version=11;",
        )
        .unwrap();
        drop(db);
        let mut store = Storage::open(&root.path().join("data")).unwrap();
