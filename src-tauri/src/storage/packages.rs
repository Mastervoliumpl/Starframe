...
impl Storage {
    ...
    pub(crate) fn recover_packages(&mut self) -> Result<()> {
        ...
        tx.execute("UPDATE package_operations SET record=json_set(record, '$.status', 'failed', '$.message', 'Starframe closed before package preparation was committed. Retry the release. Partial staging was retained; no game files changed.') WHERE json_extract(record, '$.status') IN ('preparing', 'cancelling')", [])?;
        tx.commit()?;
        Ok(())
    }
