...
fn catalog_migration_and_failed_replacement_retain_records() {
    ...
    store
        .conn
        .execute_batch("DROP TABLE app_updates; DROP TABLE catalog_security; DROP TABLE local_watches; DROP TABLE local_sources; DROP TABLE collection_imports; DROP TABLE pending_removals; DROP TABLE package_operations; DROP TABLE prepared_artifacts; DROP TABLE catalog_cache; PRAGMA user_version=5;")
        .unwrap();
    drop(store);
    let mut store = Storage::open(root.path()).unwrap();
...
fn migrations_back_up_and_failed_migrations_roll_back() {
    ...
    store
        .conn
        .execute_batch(
            "DROP TABLE app_updates; DROP TABLE catalog_security; DROP TABLE local_watches; DROP TABLE local_sources; DROP TABLE collection_imports; DROP TABLE pending_removals; DROP TABLE preferences; DROP TABLE game_selection; DROP TABLE deployments; DROP TABLE deployment_blobs; DROP TABLE catalog_cache; DROP TABLE package_operations; DROP TABLE prepared_artifacts; PRAGMA user_version = 1;",
        )
        .unwrap();
    let backup = store.backup().unwrap();
    assert!(
