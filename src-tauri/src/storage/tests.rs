...
fn catalog_migration_and_failed_replacement_retain_records() {
    ...
    store
        .conn
        .execute_batch("DROP TABLE catalog_security; DROP TABLE local_watches; DROP TABLE local_sources; DROP TABLE collection_imports; DROP TABLE pending_removals; DROP TABLE package_operations; DROP TABLE prepared_artifacts; DROP TABLE catalog_cache; PRAGMA user_version=5;")
        .unwrap();
    drop(store);
    let mut store = Storage::open(root.path()).unwrap();
