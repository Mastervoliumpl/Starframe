...
Milestone 0.6.0 adds `advisories.json` alongside the release catalog. The validator checks its schema and retained correction history against the same Git base. Revision 1 is empty; it asserts no security findings. Follow the [review and reporting process](../docs/catalog-review.md) for exact artifacts, evidence, findings and corrections.
...
6. Merge only after the review and required checks. Once enabled, the publisher signs and verifies both targets before updating the static publication branch. GitHub/CDN caching can delay visibility.

The desktop accepts fixed HTTPS metadata and target directories and rejects redirects. TUF verifies signatures, target hashes, versions, root rotation and expiry. A refresh permits at most 40 requests and 60 seconds, with separate metadata and target size limits. Successful checks repeat after five minutes while the desktop remains open; failures back off from 30 seconds to 30 minutes. Daily publisher renewal supplies thirty-day validity. Expired, unverified or mismatched catalog information pauses new downloads, while verified library reuse remains available offline. Retained confirmed findings continue to block affected copies.
...
SQLite schema 13 retains the catalog cache and adds authenticated expiry, its catalog binding and advisory history. Both targets commit together. Malformed responses, older revisions, changed identities and storage failures retain the previous data. Changed findings advance the saved-data revision so stopped-game preparation rechecks affected selections; library and collection records are retained. Old HTTP validator fields remain readable for migration but do not establish trust. A corrupt or unsupported local cache is reported for recovery instead of silently resetting its identity history. The owned `catalog-trust` directory retains TUF rollback and root-rotation state and follows the same uninstall retention choice as the library.
