...
Milestone 0.7.0 is active. Its approved scope replaces the catalog-specific design below with the website registry, exact ModID/ReleaseID/hash references, a Steam manager session, signed release and security metadata, authenticated archive delivery, and a native Mods browser. Obsolete pre-release catalog data and exports need no migration. The existing app-update signing boundary stays separate. Release, trust and download fixtures pin website contract revision `fcd81be6667e2595166698178d5f55074c5ee92a`; read-only website main later advanced to `3f983dd08ebb30cccbb7be7cfb8030ae3549c31b` without #101's signed installation plan. Shared Rust operations serve the Tauri worker and an internal headless command. The bounded registry client, exact-reference persistence, signed trust state, manager session and archive queue are implemented on draft PR #78. Production downloads remain unavailable without an independent registry root, and the old desktop catalog remains until #88–#89 replace it. Later sections retain the 0.6.0 architecture until their affected 0.7.0 issues replace it; they do not authorize a catalog fallback.
...
Issue #20 implements the resolver in `ordering.rs` and revision-checked reordering in `mods.rs`. SQLite collection positions retain requested priority; the guarded deployment manifest records effective order separately. The Collections view shows effective positions, requested positions and the constraints responsible for adjustments. [Verification and Lua scope](docs/verification/ordering.md) record the runtime evidence. Issue #21 adds [named collection operations](docs/verification/collections.md); sharing remains #22.

Support automatic resolution and manual ordering. A collection stores the user's requested sequence; deployment records the effective sequence that the runtime will use. Show both when constraints require an adjustment. Each row has a drag handle with arrow-key, Home/End and select-then-place operation, and a right-aligned order number.
...
A verified rebuild advances the matching entry in the active locally created collection. Shared imported collections and inactive collections keep their exact references. Older library builds remain available until explicitly uninstalled. When the game is closed, the existing deployment worker applies the active collection. While it runs, the latest saved revision waits until exit. There is no DLL hot reload. Uninstall removes Starframe's managed copy and references, never the developer's source directory. Shared collections mark unmatched local builds as unresolved local requirements rather than embedding binaries or paths.

File notifications trigger source checks; startup, resume and bounded fallback scans also reconcile imported sources. No watcher traverses other Steam installations. Closing the desktop stops source watching. The Windows notification API can miss remote changes, so notifications alone cannot establish current content. [Windows directory notifications](https://learn.microsoft.com/en-us/windows/win32/api/fileapi/nf-fileapi-findfirstchangenotificationw)
...
Publish signed update artifacts and matching static updater metadata through official GitHub releases. The public verification key ships with the app; the signing key stays in the release environment. Use semantic version comparison and the selected stable/preview channel, not release timestamps. Fresh prerelease builds default to Preview and stable builds to Stable; a saved user choice persists. Preview includes stable releases and neither channel permits downgrades. The owner approved this behavior on 16 September 2026. Tauri supports this installation/update path and requires update signatures. Validate the complete release flow before shipping. [Tauri updater](https://v2.tauri.app/plugin/updater/)
