...
Milestone 0.7.0 is active. Its approved scope replaces the catalog-specific design below with the website registry, exact ModID/ReleaseID/hash references, a Steam manager session, signed release and security metadata, authenticated archive delivery, and a native Mods browser. Obsolete pre-release catalog data and exports need no migration. The existing app-update signing boundary stays separate. The website contract read-only reference is `Starframe-Web` commit `62c6a44b55bf80b7d0afe91a11815a0a42d39d07` (contract path last changed at `f5ee1b3b20b18fb5028f6f682f4c86ad8aa06998`); website #101 has not yet supplied the complete signed installation plan. Shared Rust operations now serve the Tauri worker and an internal headless command. Later sections retain the 0.6.0 architecture until their affected 0.7.0 issues replace it; they do not authorize a catalog fallback.
