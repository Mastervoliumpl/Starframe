...
The owner approved the expired-metadata behavior for #46 on 9 September 2026: pause new catalog downloads until a valid signed refresh succeeds. Installed mods remain usable offline, subject to cached confirmed security findings. Show when catalog information is stale or unknown. GitHub Actions renews the signed catalog metadata daily with thirty-day validity. The [TUF verification layer](docs/verification/catalog-authentication.md) is implemented; connecting the desktop refresh, download policy and publishing workflow remains in progress.
