...
Milestone 0.7.0 is active. Its approved scope replaces the catalog-specific design below with the website registry, exact ModID/ReleaseID/hash references, a Steam manager session, signed release and security metadata, authenticated archive delivery, and a native Mods browser. Obsolete pre-release catalog data and exports need no migration. The existing app-update signing boundary stays separate. The website contract read-only reference is `Starframe-Web` main commit `ba8803aea4b6a067c1f9c60d5eb3ac33c7358567` (contract path last changed at `fcd81be6667e2595166698178d5f55074c5ee92a`); website #101 has not yet supplied the complete signed installation plan. Shared Rust operations now serve the Tauri worker and an internal headless command. The bounded registry HTTP client and schema 15 exact-reference table are underway; neither is connected to the old desktop catalog view yet. Later sections retain the 0.6.0 architecture until their affected 0.7.0 issues replace it; they do not authorize a catalog fallback.
