...
The owner approved the expired-metadata behavior for #46 on 9 September 2026: pause new catalog downloads until a valid signed refresh succeeds. Installed mods remain usable offline, subject to cached confirmed security findings. Show when catalog information is stale or unknown. The [TUF verification layer](docs/verification/catalog-authentication.md), schema-13 atomic catalog/advisory storage and confirmed-finding checks are implemented. The [publisher](docs/catalog-publishing.md) prepares daily renewal with thirty-day validity; its workflow remains disabled until production trust setup. Connecting the desktop refresh, freshness policy and warning display remains in progress.
