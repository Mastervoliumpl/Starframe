...
The shell has six destinations and four focused native commands for live state, diagnostics and fixed external pages. It has no game or storage access. The frontend uses plain Svelte/Vite; no SvelteKit or animation framework is included. Help & logs provides an explicitly labelled 1,000-row fixture and a diagnostic workload. The launch action remains unavailable until game setup and launch are implemented.
...
Run `npx playwright install chromium` once, then `npm run test:browser` for navigation, reconnect, keyboard, text resizing and diagnostic checks. Browser fixtures require the development URL `/?fixture`; production builds omit the fixture transport. These tests do not establish native behavior. On Windows, build with `npm run tauri -- build --debug --no-bundle -- --locked`, then run `npm run test:native`. This check opens and closes its own app, temporarily enables a WebView test connection on port 9223, and tests real IPC, permissions, single-instance behavior, cancellation, reconnect and process exit. It writes screenshots and timing data under ignored `test-results/native`. The port must be free and no other Starframe instance may be running. No test connection is configured in the shipped app. See [recorded results and limits](docs/verification/desktop-state.md).
