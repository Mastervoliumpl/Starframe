...
Status: milestone 0.6.0 is complete on 17 September 2026 through PR #60 and the catalog renewal correction in PR #76. The signed `0.6.0-dev.2` development draft passed owner acceptance; catalog publication, renewal and native-client verification passed. No installer or app release is published, and no later milestone has started. See the [0.6.0 acceptance record](docs/verification/milestone-0.6.0.md); retain the earlier [0.5.0 exit evidence](docs/verification/milestone-0.5.0.md), [BepInEx acceptance](docs/verification/bepinex-plugins.md) and [verification on another Windows PC](docs/verification/pc-setup.md).
...
For 0.6.0, [the consolidated acceptance record](docs/verification/milestone-0.6.0.md) supersedes older broad checklists. Extra scaling/high-contrast/screen-reader checks are waived. Absent-WebView2 testing is replaced by inspection of the existing prerequisite error handling. The owner passed the short hosted-installer smoke. Catalog publication requires checked code on main; production publication, live client refresh and renewal passed after the approved merges. These results complete the milestone's accepted scope. Public application release still requires a separate maintainer decision.
...
The shell has six destinations and five focused native commands for live state, diagnostics, fixed external pages and game discovery. The 0.1.1 build opens bundled SQLite at sqlite/state.db under Tauri's per-user local app-data directory. It converts supported Turso data on retained copies before promotion. See [SQLite verification and recovery](docs/verification/sqlite.md). The frontend has no SQL or game access. Help & logs provides an explicitly labelled 1,000-row fixture and a diagnostic workload. Internal builds can stage runtime resources to enable setup and launch; see [launch build instructions and verification](docs/verification/game-launch.md). CI executables without those resources report that setup is unavailable.
