...
For 0.1.1, retain the existing storage/native checks and add populated legacy conversion and interruption cases. Verify that the shipped Cargo graph excludes Turso. Record comparable clean and warm Windows builds with identical toolchain, target, profiles and commands; identify cache state and separate compile time from tests. Keep VERSION at 0.1.0 until implementation version preparation. Do not weaken CI to obtain a faster result.
