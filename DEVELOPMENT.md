...
The shell has six destinations and five focused native commands for live state, diagnostics, fixed external pages and game discovery. The current 0.1.0 build opens Turso storage in Tauri's per-user local app-data directory. Milestone 0.1.1 will replace it with bundled SQLite; see the [conversion and verification plan](docs/planning/sqlite-transition.md). The frontend has no SQL or game access. Help & logs provides an explicitly labelled 1,000-row fixture and a diagnostic workload. The launch action remains unavailable until game setup and launch are implemented.
...
The test-only conversion proof runs with `cargo test --manifest-path src-tauri/Cargo.toml --locked --lib storage::sqlite_proof`. Parent tests generate pinned Turso fixtures and terminate child processes; the two ignored worker functions are invoked by those tests, not skipped recovery cases. SQLite is a dev dependency until #40. See the [proof and limits](docs/verification/sqlite-conversion.md).
