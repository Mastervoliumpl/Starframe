...
- Add a bounded TUF catalog/advisory verifier with signature, expiry, rollback and retained key-rotation tests (#46). Advisory corrections retain history; schema 13 commits both verified targets together. Download, enable and activation checks enforce retained confirmed findings, including matching local payloads. Daily renewal with thirty-day validity is approved; desktop refresh, freshness enforcement, warning display and publishing remain in progress.
