...
The completed internal **0.4.0** build supports curated mod downloads, named collections, dependency ordering, drag priority and exact-reference collection sharing. Imports verify cached files and prepare missing approved releases. Saved changes deploy automatically while the game is closed, and collection switches retain mod settings. See [milestone evidence and known limits](docs/verification/milestone-0.4.0.md), [collection controls](docs/verification/collections.md) and [sharing recovery](docs/verification/sharing.md). The initial catalog is empty, and no installer or app release has been published.

The desktop uses Tauri, Svelte, Rust and local SQLite storage. Settings locates Sanctuary, saves one installation and observes its build and running state. Library records and deployment operations have recovery checks. Help & logs includes a diagnostic workload for progress, cancellation and responsiveness. Internal builds with staged runtime resources can finish setup and launch the game; see [launch setup](docs/verification/game-launch.md), [development setup](DEVELOPMENT.md#desktop-development) and [version preparation](DEVELOPMENT.md#versions-and-change-history). The four identity/runtime corrections in 0.4.1, local imports and installer delivery remain planned.
